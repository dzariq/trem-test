...
export function ObjectivesEditor({ objectives, onChange, isEditMode = true }: ObjectivesEditorProps) {
  const addObjective = () => {
    onChange([...objectives, ""]);
  };
          ...
          {objectives.map((objective, index) => (
            <div key={index} className="flex items-start gap-2">
              ...
              {isEditMode ? (
                ...
                <span className="text-sm py-2">{objective || <span className="italic text-muted-foreground">Empty objective</span>}</span>
              )}
            </div>
          ))}
